...
for col in collections:
    ...
    for plot in trackProperties + (fullTrackProperties if col == "tracks" else []):
        ...
        for r, run in enumerate(selectedRuns):
            ...
            for i, filename in enumerate(runs[run]['files']):
                ...
                dir = "/DQMData/Run %s/HLT/Run summary/Tracking/%s/"%(runs[run]['dirs'][i],col)
            ...
            if col == "tracks" and "NumberOfTracks" in plot:
                ...
                hists[plot+"_"+run].GetXaxis().SetRangeUser(0,25000)
            elif "Chi2oNDF_GenTk" in plot:
                hists[plot+"_"+run].GetXaxis().SetRangeUser(0,15)
            if r == 0:
                ...
                if any(p in plot for p in ['TrackEta', 'TrackPt', 'TrackPhi']):
                    hists[plot+"_"+run+"_ratio"].GetYaxis().SetRangeUser(0.9,1.1)
